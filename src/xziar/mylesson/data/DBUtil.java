...
public class DBUtil
{
	...
	private static final String insertSQL = "insert into lessons"
			+ "(Name,Place,Teacher,TimeFrom,TimeLast,TimeWeek,WeekFrom,WeekTo,Color) "
			+ "values(?,?,?,?,?,?,?,?,?)";
	private static final String deleteSQL = "delete from lessons where LID=?";
	private static final String selectAllSQL = "select "
			+ "LID,Name,Place,Teacher,TimeFrom,TimeLast,TimeWeek,WeekFrom,WeekTo,Color "
			+ "from lessons";
	...
	static public void add(LessonBean lb)
	{
		...
		stmt.bindString(3, lb.teacher);
		stmt.bindLong(4, lb.timeFrom);
		stmt.bindLong(5, lb.timeLast);
		stmt.bindLong(6, lb.timeWeek);
		stmt.bindLong(7, lb.weekFrom);
		stmt.bindLong(8, lb.weekTo);
		stmt.bindLong(9, lb.color);
		try
		{
	...
	static public LessonBean[] query()
	{
		...
		for(int a=0;a<cnt;a++)
		{
			...
			lbs[a].teacher = cursor.getString(3);
			lbs[a].timeFrom = cursor.getInt(4);
			lbs[a].timeLast = cursor.getInt(5);
			lbs[a].timeWeek = cursor.getInt(6);
			lbs[a].weekFrom = cursor.getInt(7);
			lbs[a].weekTo = cursor.getInt(8);
			lbs[a].color = cursor.getInt(9);
			cursor.moveToNext();
		}
		return lbs;
	}
